...
import { Instagram, CheckCircle, Loader2, Facebook } from 'lucide-react';
...
export const InstagramConnectDialog = ({ onConnected, children }: InstagramConnectDialogProps) => {
  const [open, setOpen] = useState(false);
  const dispatch = useAppDispatch();
  const { data: credentials, loading } = useAppSelector((state: RootState) => state.credentials);
  ...
  useEffect(() => {
    if (loading === 'succeeded' && isConnected) {
      onConnected();
    }
  }, [loading, isConnected, onConnected]);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          <DialogTitle className="font-headline text-2xl">Connect Instagram Business</DialogTitle>
          <p className="text-sm text-muted-foreground">
            Click the button to connect your Meta account. This will automatically configure your credentials for Instagram.
          </p>
        </DialogHeader>
          ...
          {loading === 'pending' ? (
            ...
            <div className="flex flex-col items-center justify-center text-center">
              ...
              <p className="text-muted-foreground mt-2">
                Your Instagram Business account is successfully linked.
              </p>
